...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 0.1 Set Favicon
    if (content.brand.favicon) {
        const faviconLink = document.querySelector('link[rel="icon"]');
        if (faviconLink) {
            faviconLink.href = content.brand.favicon;
            // Se for PNG/ICO, ajusta o type se necessário (removendo se for SVG)
            if (content.brand.favicon.endsWith('.png')) faviconLink.type = 'image/png';
            if (content.brand.favicon.endsWith('.ico')) faviconLink.type = 'image/x-icon';
        }
    }
